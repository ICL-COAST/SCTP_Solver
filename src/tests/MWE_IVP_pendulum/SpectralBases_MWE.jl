...
export build_quadrature, create_basis_set
using FastGaussQuadrature
    ...
    function create_basis_set(tau::Vector{<:AbstractFloat}, order::Int, type::String)

        #=
        This function computes the first [order + 1] basis functions of the Legendre and
        Chebyshev polynomial bases, as well as their derivatives at the time instances [tau].
        ...
        Returns arrays of size (order + 1, size(tau)). Each row of the output array contains
        the value of the basis functions and their derivatives at each value of tau.
        =#
        if type == "Legendre"
