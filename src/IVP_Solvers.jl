...
function DOPRI853_solve(x0, t_span, rtol, atol, type)

    if type == "Cowell"
        r, v = kepler_to_cartesian(x0)
        x0_cartesian = [r[1], r[2], r[3], v[1], v[2], v[3]]
        prob = ODEProblem(Cowell, x0_cartesian, t_span);
        sol = DifferentialEquations.solve(prob, dop853(), reltol=rtol, abstol=atol);
        #println("sol destats dopri")
        #println(sol.destats.nf)
        solution = mapreduce(permutedims, vcat, sol.u) 
        solution = cartesian_to_kepler_array(solution)
        time_s = sol.t 
...
function Vern8_solve(x0, t_span, rtol, atol, type)

    if type == "Cowell"
        r, v = kepler_to_cartesian(x0)
        x0_cartesian = [r[1], r[2], r[3], v[1], v[2], v[3]]
        prob = ODEProblem(Cowell, x0_cartesian, t_span);
        sol = DifferentialEquations.solve(prob, Vern8(), reltol=rtol, abstol=atol, progress=true, progress_steps = 1);
        solution = mapreduce(permutedims, vcat, sol.u) 
        solution = cartesian_to_kepler_array(solution)
        time_s = sol.t 
...
=#
    if type == "Cowell"
        r, v = kepler_to_cartesian(x0)
        x0_cartesian = [r[1], r[2], r[3], v[1], v[2], v[3]]
        #Cowell and Cowell_Simple as inputs are just functions for the state space described in the Cowell formulation
...
function MCPI_solve(x0, t_span, interval, rel_tolerance, it_tolerance, max_iter, type)
    if type == "Cowell"
        r, v = kepler_to_cartesian(x0)
        x0_cartesian = [r[1], r[2], r[3], v[1], v[2], v[3]]
        time_array, coefficient_array, iteration_matrix, error_matrix = ssolve_MCPI_IVP(Cowell, Cowell_simple, x0_cartesian, t_span, interval, m_order_IVP, rel_tolerance, it_tolerance, max_iter, h_adaptive)

    elseif type == "Equinoctial"
        x0_equinoctial = kepler_to_equinoctial(x0)
        time_array, coefficient_array, iteration_matrix, error_matrix = ssolve_MCPI_IVP(Equinoctial, Equinoctial_simple, x0_equinoctial, t_span, interval, m_order_IVP, rel_tolerance, it_tolerance, max_iter, h_adaptive)
    end
