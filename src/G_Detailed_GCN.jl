...
global order_array = [10,15,20,25,30]
global interval_array = [10,15,20,25,30] #keep these two the same length

global call_structure = zeros(length(order_array)+1,3) #init, rk45 and then acc, last row for temp storage

for i = 1:length(order_array)
    global s_order_IVP = order_array[i]
    global s_interval_IVP = time_span_IVP[2]./(interval_array[i])
    ...
    dev_array[spectral_interval_i,3 + method_order_i- offset_g] = abs(gcn_e_1[2].*norm_distance_E)

    println("Finishing analysing interval length,"*string((spectral_interval_i*spec_mult)))
end
